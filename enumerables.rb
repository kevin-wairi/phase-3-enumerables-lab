...
def get_names(spicy_foods)
  # your code here
  spice_names = spicy_foods.map do |spice|
    "#{spice[:name]}"
  end
   spice_names
end
...
def spiciest_foods(spicy_foods)
  # your code here
  hot_spices = spicy_foods.filter do |food|
    food[:heat_level] > 5
  end
  return hot_spices
end
...
def print_spiciest_foods(spicy_foods)
  # your code here
    filtered = spicy_foods.filter { |food| food[:heat_level].to_i > 5}

    filtered.map do |food| 
      puts ("#{food[:name]} (#{food[:cuisine]}) | Heat Level: "+"🌶" * "#{food[:heat_level]}".to_i)
    end
  end
...
def average_heat_level(spicy_foods)
  ...
  total_heat = heat_levels.compact.sum
